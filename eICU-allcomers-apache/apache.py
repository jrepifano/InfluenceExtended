...
ds = pd.read_csv('E:/Documents/GitHub/InfluenceExtended/eICU-allcomers-apache/data/nopf.csv')
...
patients = np.sort(pd.unique(ds['patientunitstayid']))
patients2 = np.sort(pd.unique(apach['patientunitstayid']))
...
for train_index,test_index in kf.split(aps):
    ...
    x_train = scaler.fit_transform(aps[train_index])
    y_train = y[train_index]
    
    x_test = scaler.transform(aps[test_index])
    y_test = y[test_index]
    ...
    pd.DataFrame(clf1.cv_results_).to_csv('results/tuning_params/smote/aps/logreg_cv_results_'+str(i)+'.csv')
...
np.save('results/probs/smote/aps/logreg_probs.npy',logreg_probs)
np.save('results/probs/smote/aps/logreg_test_labels.npy',test_labels)